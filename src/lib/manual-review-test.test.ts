...
describe("manual-review-test", () => {
  it("builds a dry-run new review event for dashboard and LINE preview", async () => {
    ...
    expect(result.savedReview.status).toBe("PENDING");
    expect(result.savedReview.aiReplyDraft).toContain("青葉ゼミナール 本校");
    ...
    expect(JSON.stringify(result.lineNotification)).toContain("✏️ 編集して返信を投稿");
